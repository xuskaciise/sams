...
import { getDeanDepartmentIds, lecturerDeanWhere } from "@/lib/dean-scope";
...
// lecturerDeanWhere — reused here exactly as everywhere else dean-scoped,
// never duplicated). A DEAN+ADMIN multi-role user is still treated as a
// Dean for this check, same "ownership-check-IS-the-query" spirit as the
// rest of the dean-scoped code: an out-of-scope department or lecturer
// simply gets rejected, not silently widened.
export async function createDailyLogEntry(input: DailyLogEntryInput) {
  ...
  // lecturer's name, which is also validated as real (and, for a Dean,
  // within their own faculty) before the entry is ever created.
  let title = data.title ?? "";
  let relatedLecturerId: string | null = null;
  if (data.type === "LEAVE_NOTICE") {
    const lecturer = await prisma.lecturer.findFirst({
      where: {
        id: data.relatedLecturerId,
        ...(isDean ? lecturerDeanWhere(deptIds) : {}),
      },
      include: { user: true },
    });
    if (!lecturer) {
